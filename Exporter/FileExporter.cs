...
namespace Exporter
{
    internal class FileExporter<T> : IFileExporter<T>
    {
        ...
        private void AddInputHeader(ExcelWorksheet worksheet)
        {
            ...
            {
                var address = ExcelHelper.GetAddress(lookup.Value.Column, 1);
        ...
        private void SetInputValues(ExcelWorksheet ws, T item, int rowNum)
        {
            foreach (var lookup in _propertyLookup)
            {
                ...
                ws.SetValue(ExcelHelper.GetAddress(lookup.Value.Column, rowNum), value);
            }
        }
    }
}
